...
namespace propulsion {
	...
	dynamics::Wrench_T<T> compute_propulsor_loads_T(
	    ...
	    const T& thrust,
	    const PropellerOmegaState_T<T>& propeller_state)
	{
		...
		if (propulsor.propellers.has_value()) {
			...
			const constants::Vector3_T<T> H_prop =
			    n_prop * (T(propellers.spin_inertia * propellers.spin_sign) * propeller_state.omega);
	    ...
	    const PropellerOmegaStateSet_T<T>& propeller_state)
	{
		...
		const dynamics::Wrench_T<T> front = compute_propulsor_loads_T<T>(propulsors.front_propulsor,
		    center_of_gravity,
		    vehicle_twist,
		    atmosphere,
		    u.front_propulsor_cmd,
		    propeller_state.front_propulsor);
		const dynamics::Wrench_T<T> left = compute_propulsor_loads_T<T>(propulsors.left_propulsor,
		    center_of_gravity,
		    vehicle_twist,
		    atmosphere,
		    u.left_propulsor_cmd,
		    propeller_state.left_propulsor);
		const dynamics::Wrench_T<T> right = compute_propulsor_loads_T<T>(propulsors.right_propulsor,
		    center_of_gravity,
		    vehicle_twist,
		    atmosphere,
		    u.right_propulsor_cmd,
		    propeller_state.right_propulsor);
