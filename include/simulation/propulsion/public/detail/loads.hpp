...
namespace propulsion
{
	...
	template <typename T>
	T compute_propeller_torque_T(const PropulsorEffector& propulsor_effector,
		const T& omega,
		const atmospheric::AirDensity& rho);

	template <typename T>
	dynamics::Wrench_T<T> compute_propulsor_loads_T(const PropulsorEffector& propulsor_effector,
	...
	template <typename T>
	dynamics::Wrench_T<T> compute_propulsive_loads_T(const PropulsorEffectors& propulsor_effectors,
		const constants::Vector3_T<T>& pB_GB,
		const dynamics::Twist_T<T>& twist,
		const atmospheric::StaticAtmosphericState& atm,
	...
	dynamics::Wrench_T<double> compute_propulsive_loads(const PropulsorEffectors& propulsor_effectors,
