...
namespace aerodynamics {
    ...
    template <typename T>
    dynamics::Wrench_T<T> step_aero_forces_moments_T(const std::vector<Surface>& surfaces, const constants::Vector3_T<T>& pB_GB, const dynamics::Twist_T<T>& twist, const atmospheric::StaticAtmosphericState& atm, const actuators::SurfaceActuatorInputs_T<T>& u, const atmospheric::Wind& windB);

    dynamics::Wrench step_aero_forces_moments(const std::vector<Surface>& surfaces, const structural::CenterOfGravity& pB_GB, const dynamics::RigidBodyState& X, const atmospheric::StaticAtmosphericState& atm, const actuators::SurfaceActuatorInputs_T<double>& u, const atmospheric::Wind& windB);
