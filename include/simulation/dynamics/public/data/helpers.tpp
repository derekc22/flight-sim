...
namespace dynamics {
	...
	template <typename T>
	State_T<T> pack_state_T(
	    const StateVector_T<T>& x)
	{
		return {.vx = x(0), .vy = x(1), .vz = x(2), .p = x(3), .q = x(4), .r = x(5), .phi = x(6), .theta = x(7)};
	}

	template <typename T>
	StateVector_T<T> unpack_state_T(
	    const State_T<T>& x)
	{
		StateVector_T<T> out;
	...
	template <typename T>
	StateDotVector_T<T> unpack_state_dot_T(
	...
	template <typename T>
	WrenchVector_T<T> unpack_wrench_T(
	...
	template <typename T>
	Wrench_T<T> pack_wrench_T(
	    const WrenchVector_T<T>& wrench)
	{
		return {.F = constants::Vector3_T<T>(wrench(0), wrench(1), wrench(2)),
		    .M = constants::Vector3_T<T>(wrench(3), wrench(4), wrench(5))};
	}

	template <typename T>
	Twist_T<T> build_twist_from_state_T(
	    const State_T<T>& x)
	{
		Twist_T<T> twist;
