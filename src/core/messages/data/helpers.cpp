...
namespace messages {
	...
	FlightGearMessageIn process_in_msg(
	    const geography::GeographicState& geo,
	    const dynamics::EulerAngles& attitude)
	{
		return FlightGearMessageIn{.altitude_ft = util::m_to_ft(geo.alt.data),
	...
	ProcessedFlightGearMessageOut process_out_msg(
	    const FlightGearMessageOut& out_msg)
	{
		atmospheric::Wind wind = atmospheric::build_wind(out_msg.wind_heading_deg, out_msg.wind_speed_kt);
