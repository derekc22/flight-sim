...
namespace devices {
	...
	static double normalize_axis(
	    Sint16 value)
	{
		return value >= 0 ? static_cast<double>(value) / SDL_JOYSTICK_AXIS_MAX
		                  : static_cast<double>(value) / -SDL_JOYSTICK_AXIS_MIN;
	}

	static double normalize_trigger(
	    Sint16 value)
	{
		return std::clamp(static_cast<double>(value) / SDL_JOYSTICK_AXIS_MAX, 0.0, 1.0);
	}

	static double apply_deadzone(
	    double value)
	{
	...
	static double button_rate(
	    SDL_Gamepad* gamepad,
	    SDL_GamepadButton increase,
	    SDL_GamepadButton decrease)
	{
		return static_cast<double>(SDL_GetGamepadButton(gamepad, increase)) -
		    static_cast<double>(SDL_GetGamepadButton(gamepad, decrease));
	}

	static double map_axis(
	    double input,
	    double limit_min,
	    double limit_max)
	{
	    ...
	    : actuator_limits(actuator_limits)
	{
		...
		for (int i = 0; i < count; ++i) {
			if (SDL_GetGamepadTypeForID(gamepads[i]) != SDL_GAMEPAD_TYPE_PS4) {
				continue;
			}
	...
	JoystickOutputRaw JoystickManager::poll()
	{
		...
		JoystickOutputRaw raw_output{
		    ...
		    .front_propulsor_rate = normalize_trigger(SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_RIGHT_TRIGGER)) -
		        normalize_trigger(SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_LEFT_TRIGGER))};
		return raw_output;
	}
	    ...
	    const JoystickManagerInput& input)
	{
		...
		u_cmd.surface_inputs.elevator_cmd =
		    map_axis(raw_output.elevator, limit_min.surface_inputs.elevator_cmd, limit_max.surface_inputs.elevator_cmd);
		u_cmd.surface_inputs.aileron_cmd =
		    map_axis(raw_output.aileron, limit_min.surface_inputs.aileron_cmd, limit_max.surface_inputs.aileron_cmd);
		u_cmd.surface_inputs.rudder_cmd =
		    map_axis(raw_output.rudder, limit_min.surface_inputs.rudder_cmd, limit_max.surface_inputs.rudder_cmd);

		u_cmd.propulsor_inputs.front_propulsor_cmd =
		    step_propulsor(input.u_cmd_t_1.propulsor_inputs.front_propulsor_cmd,
		...
		u_cmd.propulsor_inputs.left_propulsor_cmd = step_propulsor(input.u_cmd_t_1.propulsor_inputs.left_propulsor_cmd,
		    raw_output.left_propulsor_rate,
		    limit_min.propulsor_inputs.left_propulsor_cmd,
		    limit_max.propulsor_inputs.left_propulsor_cmd);
		u_cmd.propulsor_inputs.right_propulsor_cmd =
		    step_propulsor(input.u_cmd_t_1.propulsor_inputs.right_propulsor_cmd,
