...
namespace json {
	...
	guidance::TrajectoryComponents& parse_attitude_control_setpoint(
	...
	guidance::TrajectoryComponents& parse_linear_quadratic_control_setpoint(
	    const nlohmann::json& guidance_json,
	    const control::ControllerType& controller_type,
	    guidance::TrajectoryComponents& traj_components)
	{
	...
	guidance::TrajectoryType map_trajectory_type(
	    const std::string& trajectory_type_str)
	{
		if (trajectory_type_str == "regulation") {
			return guidance::TrajectoryType::Regulation;
		}
		if (trajectory_type_str == "tracking") {
			return guidance::TrajectoryType::Tracking;
		}
		if (trajectory_type_str == "interpolated") {
			return guidance::TrajectoryType::Interpolated;
		}
		throw std::runtime_error("json::map_trajectory_type unknown trajectory type: " + trajectory_type_str);
	}

	void fill_missing_trajectory_components(
	    guidance::TrajectoryComponents& traj_components)
	{
		if (traj_components.n_rows <= 0) {
			throw std::runtime_error("json::fill_missing_trajectory_components: trajectory must have at least one row. "
			                         "Possibly due to mismatched guidance and control configuration JSONs.");
		}
		if (traj_components.v_traj.size() == 0) {
			traj_components.v_traj = Eigen::MatrixXd::Zero(traj_components.n_rows, 3);
		}
		if (traj_components.w_traj.size() == 0) {
			traj_components.w_traj = Eigen::MatrixXd::Zero(traj_components.n_rows, 3);
		}
		if (traj_components.eul_traj.size() == 0) {
			traj_components.eul_traj = Eigen::MatrixXd::Zero(traj_components.n_rows, 3);
		}
	}
	...
	guidance::GuidanceManager parse_guidance_manager(
	    ...
	    const control::ControlManager& control_manager)
	{
		...
		if (config.contains("linear_quadratic")) {
			...
			traj_components = parse_linear_quadratic_control_setpoint(
			    linear_quadratic_guidance_json, controller_type, traj_components);
		}
		...
		if (config.contains("nonlinear")) {
			...
			traj_components =
			    parse_nonlinear_control_setpoint(nonlinear_guidance_json, controller_type, traj_components);
		}
