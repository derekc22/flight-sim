...
namespace json {
        ...
        for (const auto& [key, value] : run_config.items()) {
            if (!value.is_string()) { 
                throw std::runtime_error(
                    std::format("json::dump_run_configs: expected string path for key '{}'", key)
                );
            }
    ...
    void dump_analyze_configs(const std::string& dir_path) {
        ...
        const std::array<nlohmann::json, 2> cfgs{ 
            analyze_config.at("linear"),
            analyze_config.at("nonlinear")
        };
