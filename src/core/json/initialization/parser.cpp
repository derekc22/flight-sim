...
namespace json {
	    ...
	    bool trim_flag)
	{
		...
		if (config.contains("NEDFrameECEF")) {
			opts.NEDFrameECEFStepOpts = parse_NEDFrameECEF_step_options(config.at("NEDFrameECEF"));
		}
		if (config.contains("FRDFrameECEF")) {
			opts.FRDFrameECEFStepOpts = parse_FRDFrameECEF_step_options(config.at("FRDFrameECEF"));
		}
		if (config.contains("FRDFrameNED")) {
			opts.FRDFrameNEDStepOpts = parse_FRDFrameNED_step_options(config.at("FRDFrameNED"));
		}
		return opts;
	}
