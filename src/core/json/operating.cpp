...
namespace json {
    ...
    void validate_operating_properties(const operating::OperatingProperties& operating_properties, const actuators::ActuatorProperties& actuator_properties) {
        validate_fixed_control(operating_properties.fixed_actuator_inputs.flap, actuator_properties.surface_actuators.flap, "flap");
        validate_fixed_control(operating_properties.fixed_actuator_inputs.spoiler, actuator_properties.surface_actuators.spoiler, "spoiler");
    }
    ...
    operating::OperatingProperties parse_operating_properties(const nlohmann::json& config) {
        ...
        if (!config.contains("fixed_actuator_inputs")) { return operating_properties; }

        const auto& fixed_actuator_inputs_json = config.at("fixed_actuator_inputs");
        validate_fixed_actuator_inputs_json(fixed_actuator_inputs_json);

        if (fixed_actuator_inputs_json.contains("flap")) {
            operating_properties.fixed_actuator_inputs.flap = util::deg_to_rad(fixed_actuator_inputs_json.at("flap").get<double>());
        }

        if (fixed_actuator_inputs_json.contains("spoiler")) {
            operating_properties.fixed_actuator_inputs.spoiler = util::deg_to_rad(fixed_actuator_inputs_json.at("spoiler").get<double>());
        }
