...
namespace io {
	...
	void stream_estimated_vehicle_transform(
	    rerun::RecordingStream& rec,
	    const dynamics::RigidBodyState& Zt)
	{
		const Eigen::Quaterniond q_conj = Zt.q.data.conjugate(); // Rerun expects qBI, not qIB
		rec.log("world/estimated_vehicle",
		    rerun::Transform3D::from_translation_rotation(
		        rerun::components::Translation3D(static_cast<float>(Zt.p.data.x()),
	...
	void stream_vehicle_model(
	    rerun::RecordingStream& rec,
	    const Eigen::Quaterniond& q_model_to_body,
	    const std::string& path)
	{
		rec.log_static(path,
		    rerun::Transform3D::from_translation_rotation(rerun::components::Translation3D(0.0f, 0.0f, 0.0f),
		        rerun::datatypes::Quaternion::from_xyzw(static_cast<float>(q_model_to_body.x()),
		            static_cast<float>(q_model_to_body.y()),
		            static_cast<float>(q_model_to_body.z()),
		            static_cast<float>(q_model_to_body.w()))));

		rec.log_static(path + std::string("/model"), rerun::Asset3D::from_file_path("assets/cessna172.glb").value);
	}

	void clip_trajectory(
	...
	void stream_vehicle_trajectory(
	    rerun::RecordingStream& rec,
	    const std::vector<rerun::Vec3D>& trajectory)
	{
	...
	void stream_estimated_vehicle_trajectory(
	    rerun::RecordingStream& rec,
	    const std::vector<rerun::Vec3D>& estimated_trajectory)
	{
		if (estimated_trajectory.size() < 2) {
			return;
		}

		rerun::LineStrip3D strip(estimated_trajectory);

		rec.log("world/estimated_trajectory",
		    rerun::LineStrips3D(strip)
		        .with_radii(rerun::Radius::ui_points(0.5f))
		        .with_colors(rerun::Color(255, 255, 0)));
	}

	void stream_flightgear_image(
	    rerun::RecordingStream& rec,
	    const cv::Mat& image)
	{
		if (image.empty()) {
		...
		rec.log("flightgear/camera",
		    rerun::Image(rerun::borrow(img.data, img.total() * img.elemSize()),
		        rerun::WidthHeight(static_cast<uint32_t>(img.cols), static_cast<uint32_t>(img.rows)),
