...
namespace io
{
		  ...
		  rec("flight-sim")
	{
		...
		camera_worker = std::thread(&RerunRecorder::run_camera_worker, this);
		worker = std::thread(&RerunRecorder::run_worker, this);
	}
	...
	void RerunRecorder::step(
		const RerunRecorderInput& input)
	{
		int t = input.t;
	...
	void RerunRecorder::stream_context(
		const RerunContext& context)
	{
		...
		if (!context.image.empty()) {
			try {
				...
				spdlog::warn("RerunRecorder: failed to stream queued FlightGear image: {}", e.what());
			}
		}
	}

	void RerunRecorder::run_worker()
	{
		while (true) {
	...
	void RerunRecorder::run_camera_worker()
	{
		...
		while (!stop_camera_worker) {
			try {
				...
				spdlog::warn("RerunRecorder: failed to capture FlightGear image: {}", e.what());
