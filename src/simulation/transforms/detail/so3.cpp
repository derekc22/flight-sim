...
namespace transforms {
    ...
    Eigen::Matrix3d eul_to_C(double a, double b, double c, EulerOrder order, RotationType type) {
        if (type == RotationType::Extrinsic) return eul_to_C_extr(a, b, c, order);
        if (type == RotationType::Intrinsic) return eul_to_C_intr(a, b, c, order);
    ...
    Eigen::Vector3d C_to_eul(const Eigen::Matrix3d& C, EulerOrder order, RotationType type) {
        if (type == RotationType::Extrinsic) return C_to_eul_extr(C, order);
        if (type == RotationType::Intrinsic) return C_to_eul_intr(C, order);
        throw std::invalid_argument("Unsupported type");
    }
