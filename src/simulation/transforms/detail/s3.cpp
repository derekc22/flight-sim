...
namespace transforms {
    ...
    Eigen::Quaterniond eul_to_quatC(double a, double b, double c, EulerOrder order, RotationType type) {
        if (type == RotationType::Extrinsic) return eul_to_quatC_extr(a, b, c, order);
        if (type == RotationType::Intrinsic) return eul_to_quatC_intr(a, b, c, order);
    ...
    Eigen::Vector3d quatC_to_eul(const Eigen::Quaterniond& qC, EulerOrder order, RotationType type) {
        if (type == RotationType::Extrinsic) return quatC_to_eul_extr(qC, order);
        if (type == RotationType::Intrinsic) return quatC_to_eul_intr(qC, order);
        throw std::invalid_argument("Unsupported type");
    }
