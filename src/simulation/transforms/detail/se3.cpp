...
namespace transforms {
	...
	Eigen::Matrix3d C_from_H(
	    const Eigen::Matrix4d& H)
	{
		return H.block<3, 3>(0, 0);
	}

	Eigen::Matrix3d R_from_H(
	    const Eigen::Matrix4d& H)
	{
		return H.block<3, 3>(0, 0);
	}

	Eigen::Vector3d d_from_H(
	    const Eigen::Matrix4d& H)
	{
		return H.block<3, 1>(0, 3);
	}

	Eigen::Vector3d p_from_H(
	    const Eigen::Matrix4d& H)
	{
	    ...
	    TransformationOrder order)
	{
		...
		if (order == TransformationOrder::RotateFirst)
			return make_HR_rotate_first(R, d);
		if (order == TransformationOrder::TranslateFirst)
			return make_HR_translate_first(R, d);
		throw std::invalid_argument("Unsupported transformation order");
	};

	Eigen::Matrix4d make_HC(
	    ...
	    TransformationOrder order)
	{
		...
		if (order == TransformationOrder::RotateFirst)
			return make_HC_rotate_first(C, d);
		if (order == TransformationOrder::TranslateFirst)
			return make_HC_translate_first(C, d);
		throw std::invalid_argument("Unsupported transformation order");
	}

	Eigen::Matrix4d make_Hinv(
	    const Eigen::Matrix4d& H)
	{
