...
namespace dynamics
{
	...
	void OrientationMatrix::set(
		const OrientationQuaternion& q)
	{
		data = transforms::quat_to_rot(q.data);
	}

	void OrientationMatrix::set(
		const EulerAngles& eul)
	{
		data = transforms::eul_to_C(
			eul.psi(), eul.theta(), eul.phi(), transforms::EulerOrder::ZYX, transforms::RotationType::Intrinsic);
	}
	...
	void HomogeneousTransformationMatrix::set(
		const OrientationMatrix& C,
		const Position& p)
	{
		...
		const OrientationQuaternion& q)
	{
		data = transforms::make_HC(
			transforms::quat_to_rot(q.data), p().data, transforms::TransformationOrder::TranslateFirst);
	}
	...
	void OrientationQuaternion::set(
		const OrientationMatrix& C)
	{
	...
	void OrientationMatrixRate::set(
		const OrientationQuaternionRate& q_dot,
		const OrientationQuaternion& q,
		const OrientationMatrix& C)
	{
	...
	void OrientationMatrixRate::set(
		const OrientationMatrix& C,
		const AngularVelocity& w)
	{
		data = ddt_CIB(C, w).data;
	}

	void OrientationQuaternionRate::set(
		const OrientationMatrixRate& C_dot,
		const OrientationMatrix& C,
		const OrientationQuaternion& q)
	{
	...
	void AngularVelocityQuaternion::set(
		const AngularVelocity& w)
	{
		Eigen::Quaterniond q;
		q.w() = 0;
