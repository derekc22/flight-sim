...
namespace dynamics {
	...
	TranslationalAcceleration ddtB_vB_BI(
	    const TranslationalVelocity& vB_BI,
	...
	Eigen::Vector3d ddtB_to_ddtI(
	    const Eigen::Vector3d& ddtB_vB,
	    const Eigen::Vector3d& vB,
	    const Eigen::Vector3d& wB_BI)
	{
		return ddtB_vB + wB_BI.cross(vB);
	}

	Eigen::Vector3d ddtI_to_ddtB(
	    const Eigen::Vector3d& ddtI_vI,
	    const Eigen::Vector3d& vI,
	    const Eigen::Vector3d& wI_BI)
	{
	...
	AngularVelocity qIB_dot_to_wB_BI(
	...
	EulerAngleRates wB_BI_to_eul_dot(
	    const AngularVelocity& wB_BI,
	...
	AngularVelocity eul_dot_to_wB_BI(
	    const EulerAngleRates& eul_dot,
	    const EulerAngles& eul)
	{
		double theta = eul.theta();
		double phi = eul.phi();
