...
namespace dynamics {
    ...
    OrientationQuaternionRate quat_kin_vel(const OrientationQuaternion& qIB, const AngularVelocityQuaternion& wq_BI){
        OrientationQuaternionRate qIB_dot = quat_kin_vel(qIB, wq_BI.wB_BI());
        return qIB_dot;
    }
    ...
    // Output: vB(t0 + dt)
    LinearAcceleration ddtB_vB_BI(const LinearVelocity& vB_BI, const AngularVelocity& wB_BI, const Mass& mass, const Force& FB_net){
    ...
    LinearVelocity trans_dyn_vel(const LinearVelocity& vB_BI_t, const AngularVelocity& wB_BI_t, const Mass& mass, const Force& FB_net_t){
        const LinearAcceleration vB_BI_dot_t = ddtB_vB_BI(vB_BI_t, wB_BI_t, mass, FB_net_t);
    ...
    RigidBodyState step_rigid_body(const RigidBodyState& xt, const Mass& mass, const InertiaTensor& J, const Force& FB_net_t, const Moment& MB_net_t){
        ...
        const Eigen::Matrix3d CIB_t = transforms::quat2rot(xt.qIB.data);
        const Eigen::Matrix3d CBI_t = CIB_t.transpose();

        // Translational dynamics in body coordinates
        const LinearVelocity vB_BI_t1 = trans_dyn_vel(xt.vB_BI, xt.wB_BI, mass, FB_net_t); 
        const Eigen::Vector3d ddtB_vB_BI_t = ddtB_vB_BI(xt.vB_BI, xt.wB_BI, mass, FB_net_t).data;           // produces a body derivative
        const Eigen::Vector3d ddtI_vB_BI_t = ddtB_to_ddtI(ddtB_vB_BI_t, xt.vB_BI.data, xt.wB_BI.data);      // produces an inertial derivative
        ...
        const AngularVelocity wB_BI_t1 = rot_dyn(xt.wB_BI, J, MB_net_t);

        // Quaternion rotational kinematics
        const OrientationQuaternion qIB_t1 = quat_kin(xt.qIB, xt.wB_BI);

        // Convert body velocity/accel to inertial for translational kinematics update on pI_BI
        const Eigen::Vector3d vI_BI_t = CBI_t * xt.vB_BI.data;
        const Eigen::Vector3d aI_BI_t = CBI_t * aB_BI_t.data;

        // Translational kinematics in inertial coordinates
        const Position pI_BI_t1 = trans_kin(xt.pI_BI, LinearVelocity { vI_BI_t }, LinearAcceleration { aI_BI_t });
    ...
    OrientationQuaternionRate CIB_dot2qIB_dot(const OrientationMatrixRate& CIB_dot, const OrientationMatrix& CIB, const OrientationQuaternion& qIB){
        AngularVelocity wB_BI = CIB_dot2wB_BI(CIB_dot, CIB) ;
    ...
    OrientationMatrixRate qIB_dot2CIB_dot(const OrientationQuaternionRate& qIB_dot, const OrientationQuaternion& qIB, const OrientationMatrix& CIB){
        AngularVelocity wB_BI = qIB_dot2wB_BI(qIB_dot, qIB);
    ...
    AngularVelocity eul_dot2wB_BI(const EulerAngleRates& eul_dot, const EulerAngles& eul) {
        ...
        double phi = eul.phi();
        Eigen::Matrix3d T = common::eul_dot2wB_BI_mat(theta, phi);
    ...
    EulerAngleRates wB_BI2eul_dot(const AngularVelocity& wB_BI, const EulerAngles& eul) {
        double theta = eul.theta();
        double phi = eul.phi();
        Eigen::Matrix3d T = common::wB_BI2eul_dot_mat(theta, phi);
    ...
    void OrientationMatrixRate::set(const OrientationQuaternionRate& q_dot, const OrientationQuaternion& q, const OrientationMatrix& C) { data = dynamics::qIB_dot2CIB_dot(q_dot, q, C).data; }
    ...
    void EulerAngleRates::set(const AngularVelocity& w, const EulerAngles& eul){ data = dynamics::wB_BI2eul_dot(w, eul).data; }

    AngularVelocity AngularVelocityQuaternion::wB_BI() const { return AngularVelocity { data.vec() }; }
    void AngularVelocityQuaternion::set(const AngularVelocity& wB_BI) { Eigen::Quaterniond q; q.w() = 0; q.vec() = wB_BI.data; data = q; }
