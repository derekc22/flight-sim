...
namespace estimation {
    ...
    KalmanState KalmanFilter::predict(const KalmanState& prev, const Eigen::MatrixXd& A, const Eigen::MatrixXd& B, const actuators::ActuatorInputsVector_T<double>& ut_1) {
        dynamics::StateVector_T<double> zN_t_bar = A * prev.z + B * ut_1;
    ...
    KalmanState KalmanFilter::correct(const KalmanState& pred, const dynamics::StateVector_T<double>& yN_t) {
        Eigen::MatrixXd Kt = pred.P * params.C.transpose() * (params.C * pred.P * params.C.transpose() + params.Q0).inverse(); // Kalman gain

        dynamics::StateVector_T<double> Lt = yN_t - params.C * pred.z; // Innovation
