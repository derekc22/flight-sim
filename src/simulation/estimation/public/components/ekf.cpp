...
namespace estimation
{
		...
		double dt)
	{
		...
		constants::MatrixX_T<double, constants::nx, constants::nx> Pt_bar = Ft * prev.Pt * Ft.transpose() + params.R;
		...
		const linearization::OutputJacobian& C)
	{
		...
		constants::MatrixX_T<double, constants::nx, constants::nx> Kt =
			pred.Pt * Ht.transpose() * (Ht * pred.Pt * Ht.transpose() + params.Q).inverse();
