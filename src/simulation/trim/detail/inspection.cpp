...
namespace trim {
	...
	TrimSolution inspect_trim(
	    vehicles::Aircraft& aircraft,
	    const autodiff::AutoDiffModel& model,
	    const atmospheric::Wind& wind)
	{

		const actuators::PropulsorActuatorInputs_T<double>& propulsor_limit_max =
		    model.actuator_limits.limit_max.propulsor_inputs;
		const actuators::PropulsorActuatorInputs_T<double>& propulsor_limit_min =
		    model.actuator_limits.limit_min.propulsor_inputs;

		const aerodynamics::AerodynamicState target_aero =
		    aerodynamics::compute_aerodynamic_state(aircraft.CGFrameFRD, aircraft.NEDFrameECEF, wind);

		const TrimProblem
		    problem{.target = TrimTarget{.beta = target_aero.beta.data,
		        ...
		        .conditions = operating::OperatingConditions{.atm = atmospheric::compute_static_atmospheric_state(
		                                                         aircraft.FRDFrameECEF, aircraft.ECEFFrame),
		            .windB = wind,
		            .steady_state = true},
		        .initial_guess =
		            operating::OperatingPoint_T<double>{.state =
		                                                    dynamics::State_T<double>{
		                ...
		                .input = actuators::ActuatorInputs_T<double>{.surface_inputs =
		                                                                 {
		                                                                     .elevator_cmd = 0,
		                                                                     .aileron_cmd = 0,
		                                                                     .rudder_cmd = 0,
		                                                                 },
		                    .propulsor_inputs = {.front_propulsor_cmd = 0.5 *
		                            (propulsor_limit_max.front_propulsor_cmd + propulsor_limit_min.front_propulsor_cmd),
		                        .left_propulsor_cmd = 0.5 *
		                            (propulsor_limit_max.left_propulsor_cmd + propulsor_limit_min.left_propulsor_cmd),
