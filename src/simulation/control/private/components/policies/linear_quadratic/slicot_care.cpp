#include "simulation/control/private/components/policies/linear_quadratic/slicot_care.hpp"

#include "simulation/control/private/components/policies/linear_quadratic/slicot_care_wrapper.h"
...
namespace control
{
		...
		const Eigen::Ref<const Eigen::MatrixXd>& Q)
	{
		...
		util::validate_square(Q, "control::solve_care_sb02md", "Q");
		...
		Eigen::MatrixXd G_work = symmetrize(G);
		Eigen::MatrixXd Q_work = symmetrize(Q);
		...
		const Eigen::Ref<const Eigen::MatrixXd>& R)
	{
		...
		util::validate_square(R, "control::solve_care", "R");
