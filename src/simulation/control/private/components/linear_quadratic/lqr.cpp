...
namespace control {
	...
	LinearQuadraticPolicyInput LinearQuadraticRegulator::make_linear_quadratic_policy_input(
	    const LinearQuadraticControlInput& input)
	{
		...
		return {.zt = zt_deviation,
		    .A_virtual = input.virtual_linearization.A_virtual,
		    .B_virtual = input.virtual_linearization.B_virtual};
	}

	VirtualControlOutput_T<double> LinearQuadraticRegulator::step(
	    const LinearQuadraticControlInput& input,
	    double)
	{
		VirtualControlOutputVector_T<double> mu_deviation = policy.step(make_linear_quadratic_policy_input(input));
