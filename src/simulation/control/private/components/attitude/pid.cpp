...
namespace control {
	...
	AttitudePID::AttitudePID(const AttitudePIDParameters& params)
	    : lateral_policy({.Kp = params.Kp_roll, .Ki = params.Ki_roll, .Kd = params.Kd_roll, .tau = params.tau}),
	      longitudinal_policy({.Kp = params.Kp_pitch, .Ki = params.Ki_pitch, .Kd = params.Kd_pitch, .tau = params.tau}),
	      vertical_policy({.Kp = params.Kp_yaw, .Ki = params.Ki_yaw, .Kd = params.Kd_yaw, .tau = params.tau}) {};

	PIDPolicyInput AttitudePID::make_pid_policy_input(
	...
	VirtualControlOutput_T<double> AttitudePID::step(
	    const AttitudeControlInput& input,
	    double dt)
	{
		VirtualControlOutput_T<double> out{};

		out.M.x() = lateral_policy.step(make_pid_policy_input(input, AttitudeAxis::Lateral), dt);

		out.M.y() = longitudinal_policy.step(make_pid_policy_input(input, AttitudeAxis::Longitudinal), dt);

		out.M.z() = vertical_policy.step(make_pid_policy_input(input, AttitudeAxis::Vertical), dt);
