...
namespace control {
    ...
    AttitudeControl::AttitudeControl(const AttitudePIDParameters& params) :
        implementation(
            [controller = AttitudePID{ params }](const AttitudeControlInput& input, double dt) mutable {
    ...
    AttitudeControl::AttitudeControl(const DamperPIDParameters& params) :
        implementation(
            [controller = DamperPID{ params }](const AttitudeControlInput& input, double dt) mutable {
                return controller.step(input, dt);
            }
        )
    {}
