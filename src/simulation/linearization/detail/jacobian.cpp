...
namespace linearization {
	...
	LocalLinearization linearize_operating_point(
	    ...
	    const operating::OperatingConditions& conditions)
	{
		...
		CppAD::eigen_vector<CppAD::AD<double>> xu_tracked =
		...
		const dynamics::StateDot_T<CppAD::AD<double>> x_dot =
		    autodiff::compute_state_dot_T<CppAD::AD<double>>(operating_point_cppad, model, conditions, constants::dt);
	...
	VirtualLocalLinearization linearize_virtual_operating_point(
	    const autodiff::AutoDiffModel& model,
	    const operating::VirtualOperatingPoint_T<double>& virtual_operating_point)
	{

		const operating::VirtualStateInputVector_T<double> xu_virtual = operating::unpack_virtual_state_input_T<double>(
		    virtual_operating_point.state, virtual_operating_point.input);

		CppAD::eigen_vector<CppAD::AD<double>> xu_virtual_tracked =
		...
		const dynamics::StateDot_T<CppAD::AD<double>> x_dot =
		    autodiff::compute_state_dot_T<CppAD::AD<double>>(virtual_operating_point_cppad, model);
		const dynamics::StateDotVector_T<CppAD::AD<double>> x_dot_vec = dynamics::unpack_state_dot_T(x_dot);
