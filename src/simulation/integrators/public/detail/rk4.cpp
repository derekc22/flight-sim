...
namespace integrators
{
		...
		double dt)
	{
		...
		const dynamics::InertiaTensor JB_G = model.struc_t.JB_G;
		...
		const dynamics::RigidBodyStateDot k1 = compute_rigid_body_state_dot(Xt, mass, JB_G, wrench_1.net);
		...
		const dynamics::RigidBodyStateDot k2 = compute_rigid_body_state_dot(X2, mass, JB_G, wrench_2.net);
		...
		const dynamics::RigidBodyStateDot k3 = compute_rigid_body_state_dot(X3, mass, JB_G, wrench_3.net);
		...
		const dynamics::RigidBodyStateDot k4 = compute_rigid_body_state_dot(X4, mass, JB_G, wrench_4.net);
