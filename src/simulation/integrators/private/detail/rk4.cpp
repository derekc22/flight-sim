...
namespace integrators {
	    ...
	    const dynamics::Wrench& WB_net_t)
	{
		...
		return {.p_dot = dynamics::TranslationalVelocity{Xt.q.data.conjugate() * Xt.v.data},
	    ...
	    double scale)
	{
		...
		return {.p = dynamics::Position{X.p.data + X_dot.p_dot.data * scale},
	    ...
	    double dt)
	{
		...
		return {.p = dynamics::Position{X.p.data + X_dot.p_dot.data * dt},
