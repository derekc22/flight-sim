...
namespace allocator
{
	...
	AllocatorManagerOutput AllocatorManager::step(
		const AllocatorManagerInput& input)
	{
		...
		if (input.u_preferred.has_value()) {
			const operating::OperatingPoint_T<double> preferred_operating_point{
				.state = input.operating_point.state, .input = input.u_preferred.value()
			};
			const dynamics::Wrench_T<double> mu_preferred = autodiff::compute_net_wrench_T<double>(
				preferred_operating_point, input.model, input.conditions, constants::dt);
		...
		if (input.u_preferred.has_value()) {
			const actuators::ActuatorInputsVector_T<double> u_preferred =
				actuators::unpack_actuator_inputs_T(input.u_preferred.value());
			trim_hessian = R;
		...
		control::ControlOutput u_constrained =
			solve_qp_constrained(solver, hessian, gradient, u_0, actuator_target, limits, input.actuator_mask);
		control::ControlOutput u_unconstrained =
			solve_qp_unconstrained(hessian, gradient, u_0, actuator_target, limits, input.actuator_mask);

		actuators::ActuatorInputsVector_T<double> u_constrained_vec =
			actuators::unpack_actuator_inputs_T(u_constrained);
		actuators::ActuatorInputsVector_T<double> u_unconstrained_vec =
			actuators::unpack_actuator_inputs_T(u_unconstrained);
		...
		for (Eigen::Index i = 0; i < u_unconstrained_vec.rows(); ++i) {
			...
			if (u_unconstrained_vec(i) < limits(i, 0) - tolerance ||
				u_unconstrained_vec(i) > limits(i, 1) + tolerance) {
		...
		return {.u = u_constrained, .delta_mu_vec_t_1 = delta_mu_vec};
	}
