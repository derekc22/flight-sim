...
namespace allocator {
    ...
    control::ControlOutput AllocatorProperties::step(const AllocatorInput& input) {
        ...
        auto [E, mu_0] = linearize_operating_point(input.model, input.operating_point, input.conditions);
    ...
    std::tuple<EffectivenessMatrix, dynamics::WrenchVector_T<double>> linearize_operating_point(autodiff::AutoDiffModel& model, const operating::OperatingPoint& operating_point, const operating::OperatingConditions& conditions) {
        ...
        const dynamics::Wrench_T<CppAD::AD<double>> W_net = autodiff::compute_net_wrench_T<CppAD::AD<double>>(xt, ut, model, conditions, constants::dt);
