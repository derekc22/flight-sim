...
namespace runner
{
	...
	RunManagerOutput RunManager::step(
		const RunManagerInput& input)
	{
	...
	void RunManager::initialize_trim(
		StepContext& context)
	{
		...
		if (json_flags.trim_flag && !trim_wrapper.trim_sol.attempted) {
			...
			if (recording_wrapper.analysis_recorder) {
				recording_wrapper.analysis_recorder->context.trim_sol = trim_output.trim_sol;
			}
			...
			if (trim_output.trim_sol.converged) {
				...
				// update analysis context
				if (recording_wrapper.analysis_recorder) {
					recording_wrapper.analysis_recorder->context.lin_sol = linearization_output.lin_sol;
					recording_wrapper.analysis_recorder->context.eig_sol = linearization_output.eig_sol;
				}
			}
		}
	}

	void RunManager::step_measurements(
		StepContext& context)
	{
	...
	void RunManager::step_estimation(
		StepContext& context)
	{
