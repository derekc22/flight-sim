...
namespace runner
{
		...
		const RecordingWrapperInput& input)
	{
		...
		if (input.scheduler.log_tick >= constants::hz) {
			// step rerun manager
			if (rerun_recorder) {
				rerun_recorder->step({.t = input.t, .data_context = data_context});
			}
		...
		const CLIOptions& cli_options)
	{
		...
		// save data
		if (data_recorder) {
			data_recorder->save(data_dir_path);
		}

		// save analysis data
		if (analysis_recorder) {
			analysis_recorder->save(data_dir_path, report_dir_path);
