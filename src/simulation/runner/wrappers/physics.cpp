...
namespace runner {
	...
	PhysicsWrapperOutput PhysicsWrapper::step(
	    const PhysicsWrapperInput& input)
	{
		...
		integrators::RK4Model rk4_model{.struc_t = input.context.struc_t,
		...
		operating::OperatingConditions rk4_conditions{
		    .atm = input.context.atm_t, .windI = input.context.windI, .steady_state = false};

		// compute forces, moments, and next-step rigid body state
		integrators::RK4Output rk4_out = integrators::step_rigid_body_rk4(
		    input.context.Xt, rk4_model, rk4_conditions, input.context.u_actual, constants::dt);

		return {.Xt1 = rk4_out.Xt1,
